...
function resolveText(
  ...
  // Evaluate @{rand(min,max)}
  return evalRandExpressions(result);
}
...
function BackgroundLayer({
  ...
  return (
    <SkImage image={image} x={sx} y={sy} width={sw} height={sh} fit="cover" />
  );
}
...
function TextLayer({
  ...
  const fs = typeof overlay.fs === 'number' && overlay.fs > 0 ? overlay.fs : 8;
  ...
  const drawY =
    va === 'middle'
      ? scaledY + (scaledH - paraH) / 2
      : va === 'bottom'
      ? scaledY + scaledH - paraH
      : scaledY;
...
export default function QuizResultCard({
  ...
  for (const question of questions) {
    ...
    if (chosenIdx !== undefined) {
      ...
      if (optionKey)
        selectedOptions[varName] = question.options[optionKey].title;
    }
  }
  ...
  if (!showResult && questions.length > 0) {
    return (
      <QuestionView question={questions[questionIdx]} onAnswer={handleAnswer} />
    );
  }
